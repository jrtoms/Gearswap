...
function job_self_command(commandArgs, eventArgs)
    ...
    if commandArgs[1]:lower() == 'aspir' then
      handle_aspir(commandArgs)
      eventArgs.handled = true
    end
end
...
-- Handling aspir spells in descending order within Gearswap.
-- Format: gs c aspir <aspir>
function handle_aspir(cmdParams)
  local spell_recasts = windower.ffxi.get_spell_recasts()
  local target = '<t>'

  if player.job_points[(res.jobs[player.main_job_id].ens):lower()].jp_spent > 549 and spell_recasts[881] < spell_latency and actual_cost(get_spell_table_by_name('Aspir III')) < player.mp then
    windower.chat.input('/ma "Aspir III" '..target..'')
  elseif spell_recasts[248] < spell_latency and actual_cost(get_spell_table_by_name('Aspir II')) < player.mp then
    windower.chat.input('/ma "Aspir II" '..target..'')
  elseif spell_recasts[247] < spell_latency and actual_cost(get_spell_table_by_name('Aspir')) < player.mp then
    windower.chat.input('/ma "Aspir" '..target..'')
  else
    add_to_chat(123,'Abort: Aspirs on cooldown or not enough MP.')
  end
end
